...
public class Player
{
	public static final float GUN_OFFSET = -0.14f;
	...
	public static final float OFFSET_X = 0.03f; //0.05f
	public static final float OFFSET_Y = 0.00f; //0.01f
	...
	public static final int DAMAGE_MIN = 70;
	public static final int DAMAGE_MAX = 80;
	public static final int MAX_HEALTH = 100;

	private static Mesh mesh;
	private static Material shotgunMaterial;
	...
	public Player(Vector3f position)
	{
		...
		if(shotgunMaterial == null)
		{
			shotgunMaterial = new Material(new Texture("shotgun.png"));
		}

		camera = new Camera(position, new Vector3f(0,0,-1), new Vector3f(0,1,0));
		rand = new Random();
		health = MAX_HEALTH;
		shotgunTransform = new Transform();
		shotgunTransform.setTranslation(new Vector3f(7,0,7));
	...
	public void update()
	{
		...
		//Gun movement
		shotgunTransform.setTranslation(camera.getPos().add(camera.getForward().normalized().mul(0.105f)));
		shotgunTransform.getTranslation().setY(shotgunTransform.getTranslation().getY() + GUN_OFFSET);

		Vector3f directionToCamera = Transform.getCamera().getPos().sub(shotgunTransform.getTranslation());
		...
		shotgunTransform.getRotation().setY(angleToFaceTheCamera + 90);
	}

	public void render()
	{
		Shader shader = Game.getLevel().getShader();
		shader.updateUniforms(shotgunTransform.getTransformation(), shotgunTransform.getProjectedTransformation(), shotgunMaterial);
		mesh.draw();
	}
