...
public class Level
{
	...
	private ArrayList<ZombieBoss> zombieBosses;
	//private Door doorLevel;
	...
	public Level(String levelName, String textureName)
	{
		...
		//material1 = new Material(new Texture("brick.png"));
		...
		//zombies.add(new Zombie(tempTransform));
	...
	public void update()
	{
		...
		for(Zombie zombie : zombies)
			zombie.update();
		for(ZombieBoss zombieBoss : zombieBosses)
			zombieBoss.update();
	...
	public Vector2f checkIntersectionsBoss(Vector2f lineStart, Vector2f lineEnd, boolean hurtMonster)
	{
		Vector2f nearestIntersection = null;
		//System.out.println("COLLISION SIZE = " + collisionPosStart.size());
		...
		for (Door door : doors) {
			...
			//System.out.println("collisionVector = " + collisionVector);
			findNearestVector2f(nearestIntersection, collisionVector, lineStart);
			//System.out.println("start = " + collisionPosStart.get(i) + "end = " + collisionPosEnd.get(i));
			//System.out.println(nearestIntersection);
			nearestIntersection = findNearestVector2f(nearestIntersection, collisionVector, lineStart);
		}
		if (hurtMonster)
		{
			Vector2f nearestMonsterIntersect2 = null;
			ZombieBoss nearestMonsterB = null;
			for (ZombieBoss zombieBoss : zombieBosses)
			{
				Vector2f monsterSize = zombieBoss.getSize();
				Vector3f monsterPos3f = zombieBoss.getTransform().getTranslation();
				...
				nearestMonsterIntersect2 = findNearestVector2f(nearestMonsterIntersect2, collisionVector, lineStart);

				if (nearestMonsterIntersect2 == collisionVector)
					nearestMonsterB = zombieBoss;
			}
			if (nearestMonsterIntersect2 != null && (nearestIntersection == null ||
					nearestMonsterIntersect2.sub(lineStart).length() > nearestIntersection.sub(lineStart).length())) {
				if (nearestMonsterB!= null)
					nearestMonsterB.damage(player.getDamage());
			}
		}
		return nearestIntersection;

	}
	public Vector2f checkIntersections(Vector2f lineStart, Vector2f lineEnd, boolean hurtMonster)
	{
		Vector2f nearestIntersection = null;
		//System.out.println("COLLISION SIZE = " + collisionPosStart.size());
		for (int i = 0; i < collisionPosStart.size(); i++) {
		...
		if (hurtMonster)
		{
			...
			//Vector2f nearestMonsterIntersect2 = null;
			Zombie nearestMonster = null;
			//ZombieBoss nearestMonsterB=null;


			for (Zombie zombie : zombies)
			{
				Vector2f monsterSize = zombie.getSize();
				Vector3f monsterPos3f = zombie.getTransform().getTranslation();
				Vector2f monsterPos2f = new Vector2f(monsterPos3f.getX(), monsterPos3f.getZ());
				Vector2f collisionVector = lineIntersectRect(lineStart, lineEnd, monsterPos2f, monsterSize);
	...
	{
		Vector2f line1 = lineEnd1.add(lineStart1);
		//System.out.println(line1);
		Vector2f line2 = lineEnd2.add(lineStart2);
		//System.out.println(line2);
		//lineStart1 + line1 * a == lineStart2 + line2 * b

		float crossProduct = Vector2fCross(line1, line2);

		if (crossProduct == 0) {
			//System.out.println("CROSS NULL CHECK");
			return null;
		}
		Vector2f distanceBetweenLineStarts = lineStart2.sub(lineStart1);

		float a = Vector2fCross(distanceBetweenLineStarts, line2) / crossProduct;
		float b = Vector2fCross(distanceBetweenLineStarts, line1) / crossProduct;

		if (0.0f < a && a < 1.0f && 0.0f < b && b < 1.0f) {
			//System.out.println("HIT CONDITION TEST");
	...
	private void addSpecial(int blueValue, int x, int y)
	{
		...
		if(blueValue == 128)
		{
			...
			zombies.add(new Zombie(monsterTransform));
		}
		if(blueValue == 82)
		{
			Transform monsterBossTransform = new Transform();
			monsterBossTransform.setTranslation(new Vector3f((x + 0.5f) * SPOT_WIDTH, 0, (y + 0.5f) * SPOT_LENGTH));
			zombieBosses.add(new ZombieBoss(monsterBossTransform));
		}
	...
	private void generateLevel()
	{
		...
		zombieBosses = new  ArrayList<ZombieBoss>();
